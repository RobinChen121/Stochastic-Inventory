...
public class GurobiChance {
	...
	public GurobiChance(Distribution[] distributions, int[] sampleNums, double iniCash, double iniI, double[] price, double variCostUnit, 
			double salvageValueUnit, double[] overheadCost, double serviceRate) {
	...
	public double[] solve() {
		...
		try {
			...
			double[][] scenarios = sampling.generateLHSamples(distributions, sampleNums); // each row is the samples in a period
			//double[][] scenarios = {{28.0, 38.0, 40.0, 44.0, 49.0}, {22.0, 26.0, 30.0, 33.0, 42.0}};
//			for (int t = 0; t < T; t++) {
//				System.out.println(Arrays.stream(scenarios[t]).average().getAsDouble());
//			}
