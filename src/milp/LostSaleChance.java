...
public class LostSaleChance {
	...
	public double[] solveMaxSurvival() {
		...
		try {
			...
			env.set(IntParam.OutputFlag, 0); // supress warnings
			env.start();
			// Create empty model
	...
	/**
	 * Scenario model for the joint chance problem.
	 * there are anticipative constraints.
	 * @return survival probability
	 */
	public double[] solveScenario() {
		double[] result = new double[3];
		try {
			int T = distributions.length;
			int sampleNumTotal = IntStream.of(sampleNums).reduce(1, (a, b) -> a * b);
			...
			int[][][] scenarioLink = new int[T][sampleNumTotal][sampleNumTotal];
			for (int t = 0; t < T; t++) {
				int tNodeNum = IntStream.of(sampleNums).limit(t+1).reduce(1, (a, b) -> a*b);
				int tSliceNum = IntStream.of(sampleNums).skip(t+1).reduce(1, (a, b) -> a*b);
				for (int i = 0; i < tNodeNum; i++)
					for (int j = tSliceNum * i; j < tSliceNum * (i+1); j++)
						for (int k = tSliceNum * i; k < tSliceNum * (i+1); k++)
							scenarioLink[t][j][k] = 1;		
			}
			double[] scenarioProb = new double[sampleNumTotal];
			Arrays.fill(scenarioProb, 1.0/(double) sampleNumTotal);
			
			for (int t = 0; t < T; t++) 
				for (int s = 0; s < sampleNumTotal; s++) {
					double sum2 = 0;
					GRBLinExpr sum1I = new GRBLinExpr(); GRBLinExpr sum1alpha = new GRBLinExpr();
					GRBLinExpr sum1delta = new GRBLinExpr();
					for (int s1 = 0; s1 < sampleNumTotal; s1++) {
						sum2 += scenarioLink[t][s1][s] * scenarioProb[s1];
						sum1I.addTerm(scenarioLink[t][s1][s] * scenarioProb[s1], I[t][s1]);
						sum1alpha.addTerm(scenarioLink[t][s1][s] * scenarioProb[s1], alpha[t][s1]);
						sum1delta.addTerm(scenarioLink[t][s1][s] * scenarioProb[s1], delta[t][s1]);
					}
					GRBLinExpr sumIP = new GRBLinExpr(); GRBLinExpr sumDeltaP = new GRBLinExpr();
					...
					if (t > 0 && t < T - 1) {
						...
						for (int s1 = 0; s1 < sampleNumTotal; s1++) {
							sum2 += scenarioLink[t][s1][s] * scenarioProb[s1];
							sum1Q.addTerm(scenarioLink[t][s1][s] * scenarioProb[s1], Q[t+1][s1]);
						}
						GRBLinExpr sumQP = new GRBLinExpr();
						sumQP.addTerm(sum2, Q[t+1][s]);
						model.addConstr(sum1Q, GRB.EQUAL, sumQP, null);					
					}
				}		
			...
			for (int t = 0; t < T; t++) {					
				...
				Comparator<Integer> comparator = (o1, o2) -> {
					...
					if (thissSumD1 < thissSumD2) // descending
						return 1;
					else if (thissSumD1 > thissSumD2)// return -1 meaning putting the first item in the front
						return -1;
					else {
						return 0;
					}
