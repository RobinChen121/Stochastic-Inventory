...
public class ChanceCash {
	...
	public static void main(String[] args) {
		...
		int[] sampleNums = {5, 5, 5, 5, 5, 5}; // sample number in each period, the number of samples in the first period can have big influence
		double[] meanDemand = {10, 5, 10, 5, 10, 5};
		int T = sampleNums.length;
...
//		Arrays.fill(sigmas, 0.6);
		...
		int sampleNum = 30;  // simulating sample number in testing SAA and extended SAA
...
//	    result1 = simulation1.simulateSAALostRate(initialState, result[0], serviceRate, sampleNums, prices, variCostUnits, overheadCosts, salvageValueUnit, holdCostUnit, scenarios, sampleNum);
...
//		System.out.println("final simulated lost sale rate of extended SAA " + " is: " + nf.format(result1[1])); 	    
		...
		/**
		 * solve the problem by further extended formulation of SAA
		 * 
		 */
		currTime = System.currentTimeMillis();
		result = model.solveSortFurther2();	// same result with soveSortFurther2
		
		time1 = (System.currentTimeMillis() - currTime) / 1000.00;
	    currTime = System.currentTimeMillis();	    
	    System.out.println("**********************************************");
	    System.out.println("after further sorting, result of SAA-scenario tree: ");
	    System.out.println("running time is " + time1 + "s");	
	    System.out.printf("first stage decison Q is: %.2f\n", result[0]);
	    positiveScenario = result[1];
	    System.out.printf("Objective value is: %.0f in %d scenarios\n", positiveScenario, sampleNumTotal);
	    ...
	    /**
		 * Simulate the restult of further extended SAA
		 */	    
	    result1 = simulation1.simulateFurtherExtendSAALostRate2(initialState, result[0], serviceRate, sampleNums, prices, variCostUnits, overheadCosts, salvageValueUnit, holdCostUnit, scenarios, sampleNum);
		System.out.println("final simulated survival probability of further extended SAA in " + df.format(sampleNum) + " samples is: " + nf.format(result1[0]));
		System.out.println("final simulated lost sale rate of extended SAA " + " is: " + nf.format(result1[1]));
